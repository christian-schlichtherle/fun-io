...
private object ArchiveTransformationSpec {
  ...
  def forAllArchives(test: ArchiveSource[_] => ArchiveStoreFactory[_] => Any): Unit = {
    forAll(Factories)(factory => test(CC.jar(Test1JarFile))(factory))
    forAll(Factories)(factory => test(CC.jar(Test2JarFile))(factory))
  }
  ...
  private val Factories: TableFor1[ArchiveStoreFactory[_]] = Table(
    ...
    CC.jar _,
    CC.sevenz _,
    (f: File) => CC.tar(B.file(f)),
    (f: File) => CC.tar(B.file(f).map(CC.gzip)),
    CC.zip _
  )
